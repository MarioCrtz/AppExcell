...
finishTaskForm.addEventListener('submit', async (e) => {
    ...
    if (!comment) {
        alert('Por favor ingresa un comentario antes de finalizar la tarea');
        return;
    }
    ...
    if (task) {
        ...
        const ok = await updateTaskOnServer(currentTaskId, { status: 'Finalizada', comment });
        if (ok) {
            await fetchTasksFromServer();
            renderMotoristaTasks();
            renderAdminTasks();
...
document.addEventListener('DOMContentLoaded', async () => {
    // Si quieres precargar tareas desde server al abrir la app sin login
    await fetchTasksFromServer();
    renderAdminTasks();
    renderMotoristaTasks();
});
